...
import HeaderComponentAdmin from "../admin/HeaderComponentAdmin";
export default class VerPostulados extends React.Component{
    constructor(props) {
        ...
        console.log(urlParams.get('id_proyecto'));
    }
    consultar (){
        let respuesta = [];
        let datos = {
            user : JSON.parse(sessionStorage.getItem('id_proyecto'))
        }
       
        API.post('/consultar_aspirantes', datos).then(
            response => {
                let out = []; 
                JSON.parse(response.data.DATA).map(item => (
                    out.push({'id':item.id, 'cargoyarea': item.cargo+" - "+item.area , 'fecha_inicio_inscripcion':item.fecha_inicio_inscripcion, 'fecha_max_inscripcion': item.fecha_max_inscripcion, 'descripcion': item.descripcion, 'estado':item.estado, 'archivo_id': <img src={File} alt="" width="30" />, 'acciones': this.state.admin ? <div><EliminarConvocatoria data={item} /><EnlacePostulantes data={item} /></div>: <PostulanteComponent data={item} /> })
                ));
                this.setState({
                    datos: {
                        columns: [
